    ...
    >
      ...
      >
        ...
        {menuOpen && (
          ...
          >
            <button onClick={() => scrollTo("features")} className={navItemClass}>
              Features
            </button>
            <button onClick={() => scrollTo("how-it-works")} className={navItemClass}>
              How It Works
            </button>
            <button onClick={() => scrollTo("support")} className={navItemClass}>
              Healthcare Support
            </button>

            <div className="px-4 py-3">
              <span className={`flex items-center gap-3 text-sm ${darkMode ? "text-slate-300" : "text-slate-700"}`}>
                <Settings size={18} />
                Theme
              </span>
              <div
            ...
            <div className={`my-2 border-t ${darkMode ? "border-slate-700" : "border-slate-200"}`} />
      ...
      <section className="mx-auto max-w-5xl px-5 py-20 text-center">
        ...
        <p className={`mx-auto mt-4 max-w-2xl text-lg ${darkMode ? "text-slate-400" : "text-slate-600"}`}>
          Book visits, manage prescriptions, and keep every record in one
          secure place — built around how people actually manage their health.
        </p>
      ...
      <section id="features" className="mx-auto max-w-5xl px-5 py-16">
        <h2 className="text-center text-3xl font-bold">Everything care, in one app</h2>
        <div className="mt-10 grid gap-6 sm:grid-cols-2">
          {features.map(({ icon: Icon, title, desc }) => (
            <div
              key={title}
              className={`rounded-2xl p-6 ${
                darkMode ? "border border-slate-800 bg-slate-900" : "border border-slate-200 bg-white"
              }`}
            >
              <Icon className="text-cyan-400" size={28} />
              <h3 className="mt-4 font-semibold">{title}</h3>
              <p className={`mt-2 text-sm ${darkMode ? "text-slate-400" : "text-slate-600"}`}>{desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="support" className="mx-auto max-w-5xl px-5 py-16">
        ...
        >
          ...
          <div className="flex-1">
            <h3 className="text-xl font-semibold">Healthcare Support, day or night</h3>
